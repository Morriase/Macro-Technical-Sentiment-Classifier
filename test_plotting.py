...
def generate_synthetic_data(n_samples=1000, n_features=58, sequence_length=22):
    ...
    logger.info(
        f"Generating {n_samples} synthetic samples with {n_features} features")
    ...
    X[:, 10:20] = np.sin(np.linspace(0, 10*np.pi, n_samples)
                         )[:, None] + np.random.randn(n_samples, 10) * 0.5
    ...
    logger.info(
        f"Target distribution: SELL={np.sum(y == 0)}, HOLD={np.sum(y == 1)}, BUY={np.sum(y == 2)}")
...
def main():
    ...
    logger.info(
        f"Train distribution: SELL={np.sum(y_train == 0)}, HOLD={np.sum(y_train == 1)}, BUY={np.sum(y_train == 2)}")
    logger.info(
        f"Val distribution: SELL={np.sum(y_val == 0)}, HOLD={np.sum(y_val == 1)}, BUY={np.sum(y_val == 2)}")
    ...
    logger.info(
        f"Predicted distribution: SELL={np.sum(y_pred == 0)}, HOLD={np.sum(y_pred == 1)}, BUY={np.sum(y_pred == 2)}")
